...
## Walkthrough - GMSCS 플랫폼 고도화 및 고객/스토리지 정밀 통합 워크스루

## 📌 주요 조치 및 반영 결과

### 1. 고객사 다계층 스마트 분류 체계 (ClientManagement.tsx)
- **4대 스마트 세그먼트 탭바 구축**:
  - **전체 고객사 DB (573개사)**: 누적 등록된 GMSCS 인증 기업 전체 이력 데이터 보존
  - **정기유지 · 활성 고객사 (253개사 / 303개 프로젝트)**: 최근 2년 이내 주기적 사후/갱신 심사가 진행 중인 활성 유지 고객사
  - **2년 미실시 휴면·만료 대상 (320개사)**: 최근 2년 내 심사 기록이 없어 자격정지/갱신 유치가 필요한 대상 집중 관리
  - **클라우드 PDF 보관 기업 (80개사 / 790건)**: Firebase Cloud Storage 및 Firestore에 스캔본/전자문서 팩이 100% 디지털화되어 보관된 기업
- **고객사 행별 클라우드 보관 배지 (`☁ PDF N건`)**: 목록에서 즉시 클라우드 원본 보관 건수를 확인하고 클릭 시 상세 뷰어 연결

### 2. 구글 드라이브 및 원본 데이터 전수 점검 및 누락 방지 조치
- **구글 드라이브 전수 스캔 및 대조**: `G:\내 드라이브\GMSCS_과거심사보고서` 내 790건의 모든 PDF 보고서 및 인증서 파일 전수 색인 완료
- **기업명 약어 및 파일명 정밀 정규화 (209건 매핑 보정)**:
  - `케이원1공장`, `케이원2공장ohs`, `케이원메탈` ➡️ `(주)케이원메탈1공장` / `(주)케이원메탈2공장`
  - `q-한국항로표지기술원`, `oh-한국항로표지기술원` ➡️ `한국항로표지기술원`
  - `제이에스지(국문/영문)` ➡️ `(주)제이에스지`
  - `에스얜에스,qeo` ➡️ `(주)에스앤에스`
  - `태봉화장품`, `연태장업`, `지에이치테크`, `수영전자` 등 전체 공식 등록 명칭으로 100% 통합
- **Firebase Firestore `audit_documents` 및 Cloud Storage 790건 실시간 동기화 완료**

### 3. 타이포그래피 및 UI 규칙 준수
- 테이블 행 내 버튼 지양 및 **오직 기업명과 인증번호만 볼드(`font-bold`) 적용**, 그 외 텍스트는 가독성 높은 `font-normal` 유지탑재 & 고밀도 평면(목록형) 테이블 UI 전면 개편
...
### 3. **심사관리 좌측 메뉴 정리 및 6. 심사계획서 공문 입력 UI 개선**
- **2. 심사 대상 고객사 DB 검색 및 선택 메뉴 정비**:
  - 고객 등록이 '고객관리' 단일 SSOT 프로세스로 일원화됨에 따라 `[신규 등록]` 버튼 및 중복 모달 기능 제거.
  - `[이력 팝업 ↗]` 버튼 명칭을 **`[상세정보 ↗]`**로 직관적으로 변경.
- **6. 심사계획서 공문 입력 (사무국) 항목 개선**:
  - 고객의 **인증관리 담당자 성명**(`planContactPerson`) 입력 필드를 신설하여 직책과 나란히 배치.
  - 고객 선택 시 고객 DB의 담당자명(`cleanPersonName`)이 기본 자동 입력되며, 계획서 공문용으로 즉시 수정 가능하도록 연동.
  - 입력 항목 확장에 맞춰 **`추가사업장 주소 (사업장1 주소)`** 입력란을 맨 하단으로 이동하고, **1행 전체 폭(`w-full`)**을 차지하도록 배치하여 긴 주소 입력 시의 가독성과 편의성 대폭 향상.
  - 우측 F16-002 인증심사계획서 양식의 담당자 란에 실시간 반영되도록 연동 완료.
