...
import type { Company, Auditor, AuditReport, AuditContractRecord, ProofDocument, AuditProject, WeekendAuditReasonData } from '../types';
...
// ============================================================
// ISO 통합 심사보고서 팩 (Audit Report Pack: 251001 표준)
...
type DocTabKey = 'all' | 'stage1' | 'stage2' | 'cert_confirm' | 'plan_summary' | 'ncr' | 'weekend' | 'cert_change' | 'proof_upload';
...
export const AuditReportWorkbench: React.FC<AuditReportWorkbenchProps> = ({
  ...
  const [activeComp, setActiveComp] = useState<Company>(initialCompany);
  const company = activeComp;

  useEffect(() => {
    setActiveComp(initialCompany);
  }, [initialCompany]);
  ...
  // 원클릭 복사 상태 (토스트 & 인라인 툴팁)
  const [copiedField, setCopiedField] = useState<string | null>(null);
  const [copyToast, setCopyToast] = useState<string | null>(null);

  // 심사원 정보 입력 및 실시간 바인딩 상태
  const [auditorForm, setAuditorForm] = useState({
    name: auditor?.name || project?.leadAuditorName || '홍길동',
    email: auditor?.email || 'auditor@gmscs.co.kr',
    role: auditor?.grade || '선임심사원',
    regNo: auditor?.id || ''
  });

  // 사무국 검토 모드 여부
  const isReviewMode = useMemo(() => {
    if (isSecretariatReview) return true;
    if (currentUserRole === 'admin' || currentUserRole === 'office') return true;
    if (typeof window !== 'undefined' && window.location.href.includes('mode=review')) return true;
    return false;
  }, [isSecretariatReview, currentUserRole]);

  // 최종 승인 및 PDF 잠금 상태 여부
  const [isReportLocked, setIsReportLocked] = useState<boolean>(() => {
    if (typeof window !== 'undefined') {
      const compId = activeComp.id || activeComp.companyName;
      return localStorage.getItem(`gmscs_report_locked_${compId}`) === 'true' ||
             localStorage.getItem(`gmscs_report_locked_${activeComp.companyName}`) === 'true';
    }
    return false;
  });

  // 실 인증번호 계산 (DB 연동: Company.certNo, certNumber, contract.certNumber, project.certNo)
  const compAny = activeComp as any;
  const realCertNo = (activeComp as any).certNo || (activeComp as any).certNumber || (contract as any)?.certNumber || (contract as any)?.contractNumber || (project as any)?.certNo || '';

  // F19-002 인증변경신청서 데이터 State
  const [certChangeData, setCertChangeData] = useState<any>(() => {
    if (typeof window !== 'undefined') {
      try {
        const saved = localStorage.getItem(`gmscs_cert_change_${activeComp.id || activeComp.companyName}`);
    ...
    return {
      ...
      companyName: activeComp.companyName,
      ceoName: activeComp.ceoName,
      address: activeComp.address,
      tel: (activeComp as any).tel || '02-1234-5678',
      contactPerson: (activeComp as any).contactPerson || '담당자',
      standards: activeComp.standards || ['ISO 9001:2015'],
      certNumber: realCertNo || '-',
      changeTypes: {
        companyName: true,
        address: true,
        ceo: false,
      ...
      currentCompanyNameKo: activeComp.companyName,
      newCompanyNameKo: activeComp.companyName,
      newCompanyNameEn: '',
      currentAddressHeadKo: activeComp.address,
      newAddressHeadKo: activeComp.address,
      currentScope: activeComp.scope || '',
      newScope: activeComp.scope || '',
      applicantSignDate: new Date().toISOString().slice(0, 10),
      applicantSignName: activeComp.ceoName,
      reviewStatus: '서류 및 현장 확인 적합',
      reviewerName: auditorForm.name || '남경호',
      reviewDate: new Date().toISOString().slice(0, 10)
    };
  });

  const handleSaveCertChange = () => {
    if (typeof window !== 'undefined') {
      localStorage.setItem(`gmscs_cert_change_${activeComp.id || activeComp.companyName}`, JSON.stringify(certChangeData));
    }
    alert('[F19-002 인증변경신청서 저장 완료]\n현장 인증변경 신청 내역이 본 심사보고서 팩 및 시스템에 성공적으로 저장되었습니다.');
  };

  // 로컬 스토리지 키
  const storageKey = useMemo(() => `GMSCS_PACK_FULL_${activeComp.id || activeComp.companyName}`, [activeComp]);

  // 클립보드 원클릭 복사 유틸 함수
  const handleCopyText = (label: string, fieldKey: string, textToCopy: string) => {
    if (!textToCopy || textToCopy.trim() === '' || textToCopy === '-' || textToCopy.includes('미등록')) {
      alert(`[복사 안내] ${label} 항목의 유효한 데이터가 없습니다.`);
      return;
    }
    if (navigator?.clipboard?.writeText) {
      navigator.clipboard.writeText(textToCopy).then(() => {
        setCopiedField(fieldKey);
        setCopyToast(`[${label}] 클립보드에 복사되었습니다.`);
  ...
  // 심사원 정보 변경 시 보고서 본문 및 서명 슬롯 실시간 동기화
  const handleUpdateAuditorField = (field: 'name' | 'email' | 'role' | 'regNo', value: string) => {
    ...
    if (field === 'name') {
      setStage2Data((prev: any) => ({
        ...prev,
        auditorName: value,
        scheduleLeader: value,
        conflictLeader: value,
        findingsAuditor: value,
        prevObsAuditor: value,
        prevNcrAuditor: value,
      }));
      setEmailSigners(prev =>
        prev.map(s => (s.roleType === '심사팀장' ? { ...s, name: value } : s))
      );
    } else if (field === 'email') {
      setEmailSigners(prev =>
        prev.map(s => (s.roleType === '심사팀장' ? { ...s, email: value } : s))
      );
    } else if (field === 'role') {
      setEmailSigners(prev =>
        prev.map(s => (s.roleType === '심사팀장' ? { ...s, position: value } : s))
      );
    }
  };

  // 심사원 선택 드롭다운에서 심사원 선택 시
  const handleSelectAuditor = (selectedAuditorName: string) => {
    const found = auditors.find(a => a.name === selectedAuditorName);
    if (found) {
      setAuditorForm({
        name: found.name,
        email: found.email || '',
        role: found.grade || '선임심사원',
        regNo: found.id || ''
      });
      setStage2Data((prev: any) => ({
        ...prev,
        auditorName: found.name,
        scheduleLeader: found.name,
        conflictLeader: found.name,
        findingsAuditor: found.name,
        prevObsAuditor: found.name,
        prevNcrAuditor: found.name,
      }));
      setEmailSigners(prev =>
        prev.map(s => (s.roleType === '심사팀장' ? {
          ...s,
          name: found.name,
          email: found.email || s.email,
          position: found.grade || s.position
        } : s))
      );
    }
  };
  ...
  // 기업 선택 핸들러
  const handleSelectCompanyItem = (targetComp: Company) => {
    setActiveComp(targetComp);
    setIsCompanyDropdownOpen(false);
    if (onSelectCompany) {
      onSelectCompany(targetComp);
    }
  };

  // 기업 목록 필터링 (사무국: 전체 기업 / 심사원: 배정 기업)
  const isOfficeRole = currentUserRole === 'admin' || currentUserRole === 'office' || isSecretariatReview;
  const availableCompanies = useMemo(() => {
    const all = companies && companies.length > 0 ? companies : [activeComp];
    if (isOfficeRole) {
      return all;
    }
    const currentName = auditor?.name || '';
    const currentId = auditor?.id || '';
    const assigned = all.filter(c => {
      const cAny = c as any;
      const isAssigned = (cAny.assignedAuditorId && cAny.assignedAuditorId === currentId) ||
        (c.assignedAuditorName && c.assignedAuditorName.includes(currentName)) ||
        (cAny.assignedAuditor && cAny.assignedAuditor.includes(currentName)) ||
        (cAny.consultant && cAny.consultant.includes(currentName)) ||
        (cAny.leadAuditorName && cAny.leadAuditorName.includes(currentName)) ||
        (cAny.auditorHistory && Array.isArray(cAny.auditorHistory) && cAny.auditorHistory.some((h: string) => h.includes(currentName)));
      return isAssigned;
    });
    if (assigned.length > 0) return assigned;
    if (activeComp && activeComp.companyName) {
      return [activeComp];
    }
    return [];
  }, [companies, activeComp, isOfficeRole, auditor]);

  const filteredCompanies = useMemo(() => {
    if (!companySearchQuery.trim()) return availableCompanies;
    const q = companySearchQuery.toLowerCase();
    return availableCompanies.filter(c =>
      (c.companyName && c.companyName.toLowerCase().includes(q)) ||
      (c.ceoName && c.ceoName.toLowerCase().includes(q)) ||
      (c.bizNumber && c.bizNumber.includes(q))
    );
  }, [availableCompanies, companySearchQuery]);
  ...
  // 휴일근무 확인서 (F16-005) 서식 데이터 State 및 기업 전자서명 상태
  const [weekendData, setWeekendData] = useState<WeekendAuditReasonData>(() => {
    const defaultWeekend: WeekendAuditReasonData = {
      auditDates: (project as any)?.stage2StartDate ? `${(project as any)?.stage2StartDate} ~ ${(project as any)?.stage2EndDate || (project as any)?.stage2StartDate}` : (stage1Data.auditDateStart ? `${stage1Data.auditDateStart} ~ ${stage1Data.auditDateEnd || stage1Data.auditDateStart}` : '2026-10-15 ~ 2026-10-16'),
      isWeekendOrHoliday: true,
      reasonCategory: '고객사요청',
      detailedReason: '고객사의 주중 정상 조업 및 납품 생산 일정 차질을 방지하고, 교대근무 공정 및 설비 가동 상태를 온전히 확인하기 위하여 기업의 서면 요청에 따라 주말(토/일) 현장 심사를 편성하여 진행함.',
      auditorSigned: true,
      auditorSignedAt: new Date().toISOString().substring(0, 10),
      clientVerified: false,
      clientVerifiedAt: '',
      clientVerificationMethod: '전자서명',
      clientEmail: company.contactEmail || '',
      clientName: company.contactPerson ? `${company.contactPerson} (${(company as any).contactPosition || '담당자'})` : (company.ceoName ? `${company.ceoName} 대표` : '담당자'),
      signatureDataUrl: ''
    };

    if (typeof window !== 'undefined') {
      const saved = localStorage.getItem(`${storageKey}_WEEKEND`);
      if (saved) {
        try {
          return { ...defaultWeekend, ...JSON.parse(saved) };
        } catch (e) {}
      }
    }
  ...
  const [isWeekendSignatureCanvasOpen, setIsWeekendSignatureCanvasOpen] = useState<boolean>(false);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem(`${storageKey}_WEEKEND`, JSON.stringify(weekendData));
    }
  }, [signatures, emailSigners, stage1Data, stage2Data, ncrList, weekendData, storageKey]);
  ...
  // 전체 저장 & 동기화 함수 (DB/localStorage 원본 데이터 형태로만 저장, PDF 변환 금지)
  const handleSaveAll = () => {
    setIsSyncing(true);
    ...
    if (typeof window !== 'undefined') {
      ...
      // 사무국 심사진행현황에서 검토 가능한 공유 저장 플래그 설정
      localStorage.setItem(`gmscs_report_saved_${compId}`, 'true');
      localStorage.setItem(`gmscs_report_saved_${company.companyName}`, 'true');
    }
    if (onSave) {
    ...
    setTimeout(() => {
      ...
      setCopyToast('임시저장이 완료되었습니다.');
      alert(`[임시저장 완료 (${timeStr})]\n작성 중인 1·2단계 심사보고서 및 서명 데이터가 안전하게 임시저장되었습니다.`);
    }, 600);
  };

  // 보고서 제출 함수 (심사원 제출 -> 사무국 검토대기 상태 즉시 전환 및 전역 브로드캐스트)
  const handleSubmitReport = () => {
    ...
    alert(`[사무국 보고 완료]\n[${company.companyName}] 심사보고서가 사무국으로 성공적으로 보고되었습니다.\n사무국 검토대기로 전환되었습니다.`);
  };

  // 사무국 승인 (PDF 변환 & 읽기 전용 영구 보관 잠금)
  const handleOfficeApprove = () => {
    if (!confirm(`[심사보고서 최종 승인 (PDF 변환)]\n\n- ${company.companyName} 보고서를 최종 승인하시겠습니까?\n- 승인 시 현재 보고서가 PDF로 최종 렌더링/변환되어 스토리지에 저장됩니다.\n- 진행상태가 '심사 완료'로 업데이트되며, 앱 전역에서 편집이 차단된 읽기 전용으로 잠금 처리됩니다.`)) {
      return;
    }
    ...
    if (typeof window !== 'undefined') {
      ...
      localStorage.setItem(`${storageKey}_NCRS`, JSON.stringify(ncrList));
      localStorage.setItem(`${storageKey}_SCOPE_CONFIRM`, JSON.stringify(scopeConfirmData));
      localStorage.setItem(`gmscs_report_pdf_${compId}`, 'true');
      localStorage.setItem(`gmscs_report_locked_${compId}`, 'true');
      localStorage.setItem(`gmscs_report_locked_${company.companyName}`, 'true');

      // 전역 진행상태를 '심사 완료' (승인)로 갱신
      const savedStages = localStorage.getItem('gmscs_report_custom_stages');
      const stageMap = savedStages ? JSON.parse(savedStages) : {};
      stageMap[compId] = { stage: '승인', date: today, note: '사무국 최종 승인 및 PDF 변환 완료' };
      stageMap[company.companyName] = { stage: '승인', date: today, note: '사무국 최종 승인 및 PDF 변환 완료' };
      localStorage.setItem('gmscs_report_custom_stages', JSON.stringify(stageMap));
    }
    ...
    window.dispatchEvent(new CustomEvent('gmscs-report-approved', {
      detail: {
        companyId: company.id,
        companyName: company.companyName,
        status: '심사완료',
    ...
    alert(`[심사보고서 최종 승인 완료]\n\n[${company.companyName}] 심사보고서가 최종 승인되었습니다.\n보고서가 PDF로 최종 렌더링/변환되어 스토리지에 영구 보관되며 전역 읽기 전용 상태로 전환되었습니다.`);
  };

  // 사무국 반려 (보완 요청: PDF 미변환, 원본 유지, 담당 심사원에게 반려 알림 메일 발송)
  const handleOfficeReject = () => {
    const defaultLeadSigner = emailSigners.find(s => s.roleType === '심사팀장');
    const recipientEmail = defaultLeadSigner?.email || auditor?.email || 'auditor@gmscs.co.kr';
    const recipientName = defaultLeadSigner?.name || auditor?.name || '담당 심사원';

    const rejectComment = prompt(`[심사보고서 반려 (보완 요청)]\n담당 심사원(${recipientName})에게 전달할 반려 보완 요청 사유를 입력하세요:\n(본문에 기록된 '[사무국 검토 의견]'을 함께 확인 요청합니다.)`, '심사보고서 15p 심사결론 및 시정조치 내용 보완 필요');
    if (rejectComment === null) return;

    const today = new Date().toISOString().slice(0, 10);
    const compId = company.id || company.companyName;

    // 본문 심사결론 검토의견에 '[사무국 검토 의견]' 자동 명시 추가
    setStage2Data((prev: any) => ({
      ...prev,
      auditorResultReview: `${prev.auditorResultReview ? prev.auditorResultReview + '\n\n' : ''}[사무국 검토 의견 (${today})]: ${rejectComment}`
    }));

    if (typeof window !== 'undefined') {
      localStorage.setItem(`${storageKey}_STAGE1`, JSON.stringify(stage1Data));
      localStorage.setItem(`${storageKey}_STAGE2`, JSON.stringify({
        ...stage2Data,
        auditorResultReview: `${stage2Data.auditorResultReview ? stage2Data.auditorResultReview + '\n\n' : ''}[사무국 검토 의견 (${today})]: ${rejectComment}`
      }));
      localStorage.setItem(`${storageKey}_SIGS`, JSON.stringify(signatures));
      localStorage.setItem(`${storageKey}_SIGNERS`, JSON.stringify(emailSigners));
      localStorage.setItem(`${storageKey}_NCRS`, JSON.stringify(ncrList));
      ...
      // 전역 진행상태를 '보고서 보완(반려)'로 갱신하여 심사원이 재편집 가능하게 유지
      const savedStages = localStorage.getItem('gmscs_report_custom_stages');
      const stageMap = savedStages ? JSON.parse(savedStages) : {};
      stageMap[compId] = { stage: '대기', date: today, note: `보고서 반려 (사유: ${rejectComment})` };
      stageMap[company.companyName] = { stage: '대기', date: today, note: `보고서 반려 (사유: ${rejectComment})` };
      localStorage.setItem('gmscs_report_custom_stages', JSON.stringify(stageMap));
    }
    ...
    window.dispatchEvent(new CustomEvent('gmscs-report-rejected', {
      detail: {
        companyId: company.id,
        companyName: company.companyName,
        status: '보고서 보완(반려)',
    ...
    alert(`[심사보고서 반려 처리 및 알림 메일 발송 완료]\n\n• 수신자: ${recipientName} (${recipientEmail})\n• 반려 사유: ${rejectComment}\n• 안내: 본문에 기재된 [사무국 검토 의견]을 확인하고 보완 후 재제출할 수 있도록 상태가 '보고서 보완(반려)'로 업데이트되었습니다.`);
  };
  ...
  const tabConfigs = [
    ...
    },
    {
      ...
      label: '휴일근무 확인서',
      icon: Clock,
      activeColor: 'bg-white border-t-2 border-t-amber-700 text-amber-950'
    },
    {
      id: 'cert_change' as DocTabKey,
      label: '인증변경신청서',
      icon: RefreshCcw,
      activeColor: 'bg-white border-t-2 border-t-purple-700 text-purple-950'
    },
    {
    ...
    <div className="fixed inset-0 z-50 bg-slate-900/90 backdrop-blur-xs flex flex-col overflow-hidden text-slate-900">
      ...
      <div className="bg-slate-900 text-white px-5 py-2.5 flex items-center justify-between border-b border-slate-700 shrink-0 no-print print:hidden">
        ...
        <div className="flex items-center gap-2">
          ...
          {isReportLocked ? (
            <div className="flex items-center gap-2">
              <span className="px-2.5 py-1 rounded bg-amber-900/80 border border-amber-600 text-amber-200 text-xs font-bold flex items-center gap-1">
                🔒 최종 승인 완료 (읽기 전용 PDF)
              </span>
              <button
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800 text-xs font-semibold flex items-center gap-1.5 border border-slate-700 transition-colors cursor-pointer"
              >
                <span>{isOfficeRole ? '심사 목록으로 돌아가기' : '심사원 포털로 돌아가기'}</span>
              </button>
            </div>
          ) : isReviewMode ? (
            /* 사무국 검토 모드: [임시저장], [반려 (보완요청)], [승인 (PDF 변환)], [A4 인쇄], [심사 목록으로 돌아가기] */
            <div className="flex items-center gap-2">
              <span className="px-2 py-0.5 rounded text-[11px] font-bold bg-purple-900/90 text-purple-200 border border-purple-600 mr-1">
                사무국 검토 모드
              </span>
              <button
                onClick={handleSaveAll}
                disabled={isSyncing}
                className={`px-3 py-1.5 rounded-lg bg-slate-800 hover:bg-slate-700 text-slate-200 text-xs font-bold flex items-center gap-1.5 border border-slate-600 shadow-sm transition-all cursor-pointer ${
                  isSyncing ? 'animate-pulse opacity-90' : ''
                }`}
                title="사무국 검토 의견 및 수정한 내용을 원본 데이터로 임시 저장"
              >
                <Save className="w-3.5 h-3.5" />
                <span>{isSyncing ? '저장 중...' : '임시 저장'}</span>
              </button>
              <button
                onClick={handleOfficeReject}
                className="px-3.5 py-1.5 rounded-lg bg-rose-700 hover:bg-rose-600 active:bg-rose-800 text-white text-xs font-bold flex items-center gap-1.5 border border-rose-500 shadow-sm transition-colors cursor-pointer"
                title="PDF 변환 없이 원본 유지 및 담당 심사원에게 반려 메일 발송"
              >
              ...
              <button
                ...
                className="px-3.5 py-1.5 rounded-lg bg-emerald-600 hover:bg-emerald-500 active:bg-emerald-700 text-white text-xs font-bold flex items-center gap-1.5 border border-emerald-400 shadow-sm transition-colors cursor-pointer"
                title="사무국 최종 승인 및 PDF 변환 & 읽기 전용 영구 보관"
              >
              ...
              <button
                ...
                className="px-3 py-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800 text-xs font-semibold flex items-center gap-1.5 border border-slate-700 transition-colors cursor-pointer"
              >
                <span>심사 목록으로 돌아가기</span>
              </button>
            </div>
          ) : (
            /* 심사원 모드 (요청 순서): 1. [임시저장] -> 2. [사무국 보고] -> 3. [A4 인쇄] -> 4. [심사원 포털로 돌아가기] */
            <div className="flex items-center gap-2">
              <button
                onClick={handleSaveAll}
                disabled={isSyncing}
                ...
                title="현재 작성 중인 심사 내용 임시 저장 (로컬 Draft 보존)"
              >
                <Save className="w-3.5 h-3.5" />
                <span>{isSyncing ? '임시저장 중...' : '임시저장'}</span>
              </button>
              <button
              ...
              <button
                ...
                onClick={handleCloseWithGuard}
                className="px-3 py-1.5 rounded-lg text-slate-300 hover:text-white hover:bg-slate-800 text-xs font-semibold flex items-center gap-1.5 border border-slate-700 transition-colors cursor-pointer"
                title="심사원 포털 메인/목록으로 안전하게 복귀"
              >
                <span>심사원 포털로 돌아가기</span>
              </button>
            </div>
          )}
        </div>
      ...
      <div className="flex-1 flex overflow-hidden bg-slate-200/80 print:bg-white print:overflow-visible print:block">
        ...
        <div className="w-80 lg:w-96 bg-slate-50 border-r border-slate-300 p-4 space-y-4 overflow-y-auto shrink-0 text-xs no-print print:hidden">
          ...
          {/* 플로팅 클립보드 복사 알림 토스트 */}
          {copyToast && (
            <div className="bg-slate-900 text-teal-300 px-3 py-2 rounded-xl shadow-lg border border-teal-500/30 flex items-center gap-2 animate-in fade-in slide-in-from-top-2 duration-200">
              <Check className="w-4 h-4 text-teal-400 shrink-0" />
              <span className="font-medium text-[11px] leading-tight text-white">{copyToast}</span>
            </div>
          )}
          ...
          <div className="bg-slate-800 text-white px-3 py-2 rounded-xl flex items-center justify-between shadow-2xs">
            <span className="font-bold text-xs flex items-center gap-1.5">
              ...
              <span>심사 작성 정보 & 바인딩 패널</span>
            </span>
            <span className="text-[10px] text-teal-300 font-mono">Reference & Binding</span>
          </div>

          {/* 사무국 검토 모드 전용 안내 카드 */}
          {isReviewMode && (
            <div className="bg-purple-50 p-3.5 rounded-xl border border-purple-300 shadow-2xs space-y-2 text-purple-950">
              ...
              <p className="text-[11px] text-purple-900 leading-relaxed">
                사무국 담당자가 기록하는 의견은 <strong className="font-bold text-purple-950">[사무국 검토 의견]</strong> 식별자로 본문에 직접 저장됩니다.
              </p>
              <div className="flex gap-1.5 pt-1">
                <button
                  type="button"
                  onClick={() => {
                    const comment = prompt('[사무국 검토 의견 추가]\n보고서 본문(15p 심사결론)에 추가할 검토 의견을 입력하세요:');
                    if (comment) {
                      const today = new Date().toISOString().slice(0, 10);
                      setStage2Data((prev: any) => ({
                        ...prev,
                        auditorResultReview: `${prev.auditorResultReview ? prev.auditorResultReview + '\n\n' : ''}[사무국 검토 의견 (${today})]: ${comment}`
                      }));
                      alert('15p 심사결론 항목에 [사무국 검토 의견]이 성공적으로 반영되었습니다.');
                    }
                  }}
                  className="w-full py-1.5 px-2 bg-purple-700 hover:bg-purple-600 text-white rounded font-bold text-[11px] flex items-center justify-center gap-1 transition-colors cursor-pointer"
                >
                  <Edit3 className="w-3 h-3" />
                  <span>+ [사무국 검토 의견] 본문 추가</span>
                </button>
              </div>
            </div>
          )}
          ...
          <div className="bg-white p-3.5 rounded-xl border border-slate-300 shadow-2xs space-y-2.5">
            ...
            <div className="relative">
              <input
                ...
                className="w-full pl-7 pr-7 py-1.5 bg-slate-50 border border-slate-300 rounded-lg text-xs text-slate-900 placeholder-slate-400 focus:bg-white focus:border-teal-600 focus:outline-hidden"
              />
              ...
              {companySearchQuery && (
                <button
                  ...
                  className="p-1 text-slate-400 hover:text-slate-600 absolute right-1.5 top-1.5"
                >
                  <X className="w-3 h-3" />
                </button>
              )}
            ...
            <div className="space-y-1">
              <label className="text-[10px] font-bold text-slate-600 block">대상 기업 변경/선택:</label>
              <select
                value={activeComp.id || activeComp.companyName}
                onChange={(e) => {
                  const target = availableCompanies.find(c => (c.id || c.companyName) === e.target.value);
                  if (target) handleSelectCompanyItem(target);
                }}
                className="w-full px-2 py-1.5 bg-white border border-slate-300 rounded-lg text-xs font-semibold text-slate-900 focus:border-teal-600 focus:outline-hidden"
              >
                {filteredCompanies.map((c) => (
                  <option key={c.id || c.companyName} value={c.id || c.companyName}>
                    {c.companyName} ({c.ceoName ? `${c.ceoName} 대표` : '대표자 미상'})
                  </option>
                ))}
              </select>
            </div>
          ...
          {/* ======================================================== */}
          {/* 2. 기업 정보 카드 및 원클릭 복사 UI (8대 항목) */}
          {/* ======================================================== */}
          <div className="bg-white p-3.5 rounded-xl border border-slate-300 shadow-2xs space-y-2.5">
            <div className="flex items-center justify-between border-b border-slate-200 pb-1.5">
              <span className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                <span className="w-1.5 h-3 bg-teal-700 inline-block rounded-xs"></span>
                <span>2. 기업 정보 카드 (원클릭 복사)</span>
              </span>
              <span className="text-[10px] font-bold text-teal-800 bg-teal-50 px-1.5 py-0.5 rounded border border-teal-200">
                8대 핵심정보
              </span>
            ...
            <div className="space-y-2 text-[11px]">
              {/* 1. 고객사명 (국문) */}
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">1. 고객사명 (국문)</span>
                  <span className="font-bold text-slate-950 text-xs break-all select-all">
                    {activeComp.companyName || scopeConfirmData.companyNameKor || '-'}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('고객사명(국문)', 'compName', activeComp.companyName || scopeConfirmData.companyNameKor || '')}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'compName'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  }`}
                  title="고객사명 클립보드 복사"
                >
                  {copiedField === 'compName' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">2. 대표자명</span>
                  <span className="font-bold text-slate-900 break-all select-all">
                    {activeComp.ceoName || scopeConfirmData.ceoName || '-'}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('대표자명', 'ceoName', activeComp.ceoName || scopeConfirmData.ceoName || '')}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'ceoName'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  ...
                  title="대표자명 클립보드 복사"
                >
                  {copiedField === 'ceoName' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">3. 사업자등록번호 / 인증번호</span>
                  <div className="font-mono text-slate-900 space-y-0.5">
                    <div><strong className="text-slate-600 text-[10px]">사업자:</strong> {activeComp.bizNumber || '-'}</div>
                    <div><strong className="text-slate-600 text-[10px]">인증번호:</strong> {realCertNo || '-'}</div>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('사업자번호/인증번호', 'bizCert', `${activeComp.bizNumber ? `사업자: ${activeComp.bizNumber}` : ''}${realCertNo ? ` / 인증번호: ${realCertNo}` : ''}`)}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'bizCert'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  }`}
                  title="사업자번호 및 인증번호 복사"
                >
                  {copiedField === 'bizCert' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">4. 담당자명 및 연락처</span>
                  <span className="text-slate-900 font-medium break-all select-all">
                    {activeComp.contactPerson || '-'}{' '}
                    {activeComp.contactPhone ? `(${activeComp.contactPhone})` : ''}{' '}
                    {activeComp.contactEmail ? `[${activeComp.contactEmail}]` : ''}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('담당자 및 연락처', 'contact', `${activeComp.contactPerson || ''} ${activeComp.contactPhone || ''} ${activeComp.contactEmail || ''}`.trim())}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'contact'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  ...
                  title="담당자 및 연락처 복사"
                >
                  {copiedField === 'contact' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">5. 사업장 주소</span>
                  <span className="text-slate-900 leading-tight block break-all select-all">
                    {scopeConfirmData.addressKor || activeComp.address || '-'}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('사업장 주소', 'address', scopeConfirmData.addressKor || activeComp.address || '')}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'address'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  ...
                  title="사업장 주소 복사"
                >
                  {copiedField === 'address' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">6. 인증 규격 및 인증 범위 (국문)</span>
                  <div className="space-y-0.5">
                    <div className="font-bold text-teal-900 text-[10.5px]">
                      {stage1Data.auditStandards || scopeConfirmData.standards || contract?.standards?.join(', ') || 'ISO 9001 / ISO 14001'}
                    </div>
                    <div className="text-slate-800 leading-snug break-all select-all">
                      {scopeConfirmData.scopeKor || activeComp.scope || activeComp.industry || '-'}
                    </div>
                  </div>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('인증 규격 및 국문 범위', 'scopeKor', `${stage1Data.auditStandards || scopeConfirmData.standards || ''} : ${scopeConfirmData.scopeKor || activeComp.scope || activeComp.industry || ''}`)}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'scopeKor'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  }`}
                  title="인증 규격 및 국문 범위 복사"
                >
                  {copiedField === 'scopeKor' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">7. 인증 범위 (영문)</span>
                  <span className={`leading-snug block break-all select-all ${
                    scopeConfirmData.scopeEng || (activeComp as any).scopeEng ? 'text-slate-900 font-serif' : 'text-slate-400 italic'
                  }`}>
                    {scopeConfirmData.scopeEng || (activeComp as any).scopeEng || '(영문 범위 미등록)'}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('영문 인증 범위', 'scopeEng', scopeConfirmData.scopeEng || (activeComp as any).scopeEng || '')}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'scopeEng'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  }`}
                  title="영문 인증 범위 복사"
                >
                  {copiedField === 'scopeEng' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
              ...
              <div className="p-2 bg-slate-50 hover:bg-slate-100/80 rounded-lg border border-slate-200 flex items-start justify-between gap-2 transition-colors">
                <div className="space-y-0.5 min-w-0 flex-1">
                  <span className="text-[9.5px] font-bold text-slate-500 block">8. 심사 코드 (IAF / EA Code)</span>
                  <span className="font-mono font-bold text-slate-900 break-all select-all">
                    {scopeConfirmData.iafCode || (activeComp as any).iafCode || (activeComp as any).eaCode || (activeComp as any).scopeCode || 'IAF 18 / 14'}
                  </span>
                </div>
                <button
                  type="button"
                  onClick={() => handleCopyText('심사 코드', 'iafCode', scopeConfirmData.iafCode || (activeComp as any).iafCode || (activeComp as any).eaCode || (activeComp as any).scopeCode || 'IAF 18 / 14')}
                  className={`p-1.5 rounded-md border transition-all shrink-0 cursor-pointer ${
                    copiedField === 'iafCode'
                      ? 'bg-emerald-100 border-emerald-400 text-emerald-800'
                      : 'bg-white border-slate-300 hover:bg-slate-200 text-slate-700'
                  ...
                  title="심사 코드 복사"
                >
                  {copiedField === 'iafCode' ? <Check className="w-3.5 h-3.5 text-emerald-600" /> : <Copy className="w-3.5 h-3.5 text-slate-600" />}
                </button>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white p-3.5 rounded-xl border border-slate-300 shadow-2xs space-y-3">
            ...
            <p className="text-[10.5px] text-slate-500 leading-tight">
              좌측 패널에서 심사원 정보를 입력·수정하면 보고서 본문(작성자란, 일정표, 서명란 등)에 즉시 바인딩되어 반영됩니다.
            </p>

            {/* 심사원 목록에서 원클릭 선택 (있을 경우) */}
            {auditors && auditors.length > 0 && (
              <div className="space-y-1">
                <label className="text-[9.5px] font-bold text-slate-600 block">등록 심사원 빠른 선택:</label>
                <select
                  value={auditorForm.name}
                  onChange={(e) => handleSelectAuditor(e.target.value)}
                  className="w-full px-2 py-1 bg-slate-50 border border-slate-300 rounded text-xs font-semibold text-slate-900 focus:bg-white focus:border-indigo-600"
                >
                  {auditors.map((a) => (
                    <option key={a.id || a.name} value={a.name}>
                      {a.name} ({a.grade || '심사원'}) - {a.email || '이메일미등록'}
                    </option>
                  ))}
                </select>
              </div>
            ...
            <div className="space-y-2 text-[11px]">
              <div className="grid grid-cols-2 gap-2">
                <div>
                  <label className="text-[9.5px] text-slate-600 font-bold block mb-0.5">심사원명</label>
                  <input
                    type="text"
                    value={auditorForm.name}
                    onChange={(e) => handleUpdateAuditorField('name', e.target.value)}
                    placeholder="심사원명"
                    className="w-full px-2 py-1 bg-white border border-slate-300 rounded text-xs font-bold text-slate-900 focus:border-indigo-600"
                  />
                </div>
                <div>
                  <label className="text-[9.5px] text-slate-600 font-bold block mb-0.5">직급 / 역할</label>
                  <input
                    type="text"
                    value={auditorForm.role}
                    onChange={(e) => handleUpdateAuditorField('role', e.target.value)}
                    placeholder="선임심사원 / 심사원"
                    className="w-full px-2 py-1 bg-white border border-slate-300 rounded text-xs text-slate-900 focus:border-indigo-600"
                  />
                </div>
              </div>

              <div>
                <label className="text-[9.5px] text-slate-600 font-bold block mb-0.5">심사원 이메일</label>
                <input
                  type="email"
                  value={auditorForm.email}
                  onChange={(e) => handleUpdateAuditorField('email', e.target.value)}
                  placeholder="auditor@gmscs.co.kr"
                  className="w-full px-2 py-1 bg-white border border-slate-300 rounded text-xs font-mono text-slate-900 focus:border-indigo-600"
                />
              </div>

              <div>
                <label className="text-[9.5px] text-slate-600 font-bold block mb-0.5">심사원 등록번호</label>
                <input
                  type="text"
                  value={auditorForm.regNo}
                  onChange={(e) => handleUpdateAuditorField('regNo', e.target.value)}
          ...
          {/* 5. 전자 메일 서명 대상자 관리 및 확인발송 */}
          <div className="bg-white p-3.5 rounded-xl border border-slate-300 shadow-2xs space-y-3">
            <div className="flex items-center justify-between border-b border-slate-200 pb-1.5">
              <span className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                <span className="w-1.5 h-3 bg-teal-700 inline-block rounded-xs"></span>
                <span>5. 전자 메일 서명 관리 & 발송</span>
              </span>
                    ...
                    {(signer.roleType === '고객담당자' || signer.roleType === '근로자대표') && (
                      <label className="flex items-center gap-1.5 pt-0.5 cursor-pointer text-[10.5px] text-slate-700 font-medium select-none">
                        <input
                          ...
                          onChange={(e) => {
                            const next = [...emailSigners];
                            const shared = activeComp.contactEmail || (activeComp as any).email || '';
                            next[sIdx] = {
                              ...next[sIdx],
                        ...
                        <span className="text-[9.5px] text-teal-700 font-mono">({activeComp.contactEmail || (activeComp as any).email || '대표메일'})</span>
                      </label>
                    )}
          ...
          {/* 7. 보고서 작성 인증원 공지사항 (Certification Body Notice) */}
          <div className="bg-white p-3.5 rounded-xl border border-slate-300 shadow-2xs space-y-2.5">
            <div className="flex items-center justify-between border-b border-slate-200 pb-1.5">
              <span className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                <span className="w-1.5 h-3 bg-indigo-700 inline-block rounded-xs"></span>
                <span>7. 보고서 작성 인증원 공지사항</span>
              </span>
        ...
        <div className="flex-1 flex flex-col min-w-0 bg-slate-300/60 overflow-hidden print:bg-white print:overflow-visible print:w-full">
          ...
          <div className="flex-1 p-6 md:p-8 overflow-y-auto flex justify-center bg-slate-200/60 print:bg-white print:p-0 print:overflow-visible print:block">
            <div className="w-full max-w-[850px] space-y-12 print:max-w-none print:w-full print:space-y-0">
              ...
              {/* ================================================================= */}
              {/* 휴일근무 확인서 (F16-005) - 기업 전자서명/캔버스 서명 연동 */}
              {/* ================================================================= */}
                ...
                <div className="w-full bg-white border border-slate-300 shadow-md p-8 md:p-12 text-slate-900 font-sans a4-page min-h-[1100px] space-y-6 relative flex flex-col justify-between">
                  <div className="space-y-6">
                    ...
                    <div className="text-center py-3">
                      <h1 className="text-2xl md:text-3xl font-black tracking-tight text-slate-950 font-serif">
                        휴일(토/일) 및 야간 심사 사유 확인서
                      </h1>
                      <p className="text-xs text-slate-500 mt-1 font-serif">
                        (주말 및 법정 공휴일 현장 심사 수행에 따른 피심사기업 확인서)
                      </p>
                    </div>
                    ...
                    <table className="w-full border-collapse border border-slate-700 text-xs mb-4">
                      <tbody>
                        <tr className="border-b border-slate-400">
                          <th className="w-28 bg-slate-100 p-2.5 border-r border-slate-400 text-center font-bold">피심사기업</th>
                          <td className="p-2.5 font-bold text-sm text-slate-900 border-r border-slate-400">{company.companyName}</td>
                          <th className="w-28 bg-slate-100 p-2.5 border-r border-slate-400 text-center font-bold">사업자번호</th>
                          <td className="p-2.5 font-mono text-slate-900">{company.bizNumber || '-'}</td>
                        </tr>
                        <tr className="border-b border-slate-400">
                          <th className="bg-slate-100 p-2.5 border-r border-slate-400 text-center font-bold">심사팀장</th>
                          <td className="p-2.5 font-semibold text-slate-900 border-r border-slate-400">
                            {auditorForm.name} ({auditorForm.role || '선임심사원'})
                          </td>
                          <th className="bg-slate-100 p-2.5 border-r border-slate-400 text-center font-bold">심사구분</th>
                          <td className="p-2.5 font-semibold text-teal-800">
                            {stage1Data.auditType || '정기 사후관리심사'}
                          </td>
                        </tr>
                        <tr className="border-b border-slate-400">
                          <th className="bg-slate-100 p-2.5 border-r border-slate-400 text-center font-bold">적용표준</th>
                          <td colSpan={3} className="p-2.5 text-slate-800 font-medium">
                            {stage1Data.auditStandards || 'ISO 9001:2015'}
                          </td>
                        </tr>
                        <tr>
                          <th className="bg-slate-100 p-2.5 border-r border-slate-400 text-center font-bold">심사 대상일</th>
                          <td colSpan={3} className="p-2.5 font-mono text-slate-900">
                            <input
                              type="text"
                              value={weekendData.auditDates}
                              onChange={(e) => setWeekendData(prev => ({ ...prev, auditDates: e.target.value }))}
                              placeholder="YYYY-MM-DD ~ YYYY-MM-DD (주말/공휴일 포함)"
                              className="w-full bg-slate-50 border border-slate-300 rounded px-2 py-1 text-xs font-mono font-bold focus:bg-white focus:outline-none"
                            />
                          </td>
                        </tr>
                      </tbody>
                    ...
                    {/* 사유 분류 및 상세 */}
                    <div className="border border-slate-400 p-4 bg-slate-50/70 rounded-md space-y-3">
                      <div className="flex items-center justify-between">
                        <span className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                          <span>■ 휴일(주말/공휴일) 심사 수행 사유:</span>
                        </span>
                        <div className="flex items-center gap-4 text-xs font-medium">
                          {(['고객사요청', '전기요금절감', '연속가동생산', '공정특성', '기타'] as const).map(cat => (
                            <label key={cat} className="inline-flex items-center gap-1 cursor-pointer">
                              <input
                                type="radio"
                                name="weekendReasonCat"
                                checked={weekendData.reasonCategory === cat}
                                onChange={() => setWeekendData(prev => ({ ...prev, reasonCategory: cat }))}
                                className="text-teal-700 accent-teal-700"
                              />
                              <span>{cat}</span>
                            </label>
                          ))}
                      ...
                      <textarea
                        rows={5}
                        value={weekendData.detailedReason}
                        onChange={(e) => setWeekendData(prev => ({ ...prev, detailedReason: e.target.value }))}
                        placeholder="휴일 및 야간 심사 편성 사유를 구체적으로 작성하세요 (예: 기업의 생산 교대 일정, 주중 조업 차질 방지 및 기업 서면 요청 사항 등)..."
                        className="w-full border border-slate-300 rounded p-3 text-xs leading-relaxed bg-white focus:ring-1 focus:ring-teal-500 focus:outline-none"
                      />

                      <p className="text-[11px] text-slate-500 leading-tight">
                        ※ KAB 공인 심사 기준 및 노동법규 준수를 위하여, 기업의 서면 동의 및 실질 가동 상태를 확인하고 작성된 공식 증빙 서식입니다.
                      </p>
                    </div>

                    {/* 서명 확인란 (좌: 심사팀장 / 우: 피심사기업 확인(좌측 기업 정보 패널 연동)) */}
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6 pt-4 border-t-2 border-slate-400">
                      {/* 1. 심사팀장 서명 */}
                      <div className="border border-slate-300 rounded-lg p-4 bg-white text-center space-y-3">
                        <span className="font-bold text-slate-900 block text-xs">[심사팀장 확인]</span>
                        <div className="text-xs text-slate-700 space-y-1">
                          <p>소속: <strong>글로벌매니지먼트시스템인증원(주)</strong></p>
                          <p>직위: <strong>{auditorForm.role || '선임심사원'}</strong></p>
                          <p>성명: <strong className="text-sm text-slate-900">{auditorForm.name}</strong></p>
                        </div>
                        <div className="pt-2">
                          <span className="text-emerald-700 font-bold text-xs bg-emerald-50 border border-emerald-300 px-3 py-1.5 rounded inline-flex items-center gap-1 shadow-2xs">
                            <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                            <span>심사팀장 전자서명 승인 완료</span>
                          </span>
                        </div>
                      </div>

                      {/* 2. 피심사기업 확인 (기업 담당자 전자서명 연동) */}
                      <div className="border border-slate-300 rounded-lg p-4 bg-white text-center space-y-3">
                        <span className="font-bold text-slate-900 block text-xs">[피심사기업 확인]</span>
                        <div className="text-xs text-slate-700 space-y-1">
                          <p>기업명: <strong>{company.companyName}</strong></p>
                          <p>확인자: <strong className="text-slate-900">{company.contactPerson || company.ceoName || '기업 담당자'}</strong> ({(company as any).contactPosition || '담당자'})</p>
                          <p className="text-[11px] text-slate-500">
                            연락처: {company.contactPhone || '-'} {company.contactEmail ? `[${company.contactEmail}]` : ''}
                          </p>
                        </div>

                        {/* 서명 렌더링 영역 */}
                        <div className="pt-1">
                          {weekendData.signatureDataUrl ? (
                            <div className="space-y-1">
                              <img
                                src={weekendData.signatureDataUrl}
                                alt="기업 담당자 전자서명"
                                className="h-14 max-w-[200px] object-contain mx-auto border border-emerald-300 rounded bg-white p-1 shadow-2xs"
                              />
                              <span className="text-emerald-700 font-bold text-[10.5px] block">
                                ✓ 기업 담당자 캔버스 전자서명 완료 ({weekendData.clientVerifiedAt || '현장 서명'})
                              </span>
                            </div>
                          ) : weekendData.clientVerified ? (
                            <div className="space-y-1">
                              <span className="text-emerald-700 font-bold text-xs bg-emerald-50 border border-emerald-300 px-3 py-1.5 rounded inline-flex items-center gap-1 shadow-2xs">
                                <CheckCircle2 className="w-3.5 h-3.5 text-emerald-600" />
                                <span>기업 확인 승인 완료 ({weekendData.clientVerifiedAt || '확인'})</span>
                              </span>
                            </div>
                          ) : (
                            <div className="space-y-2">
                              <span className="text-amber-700 font-medium text-xs bg-amber-50 border border-amber-300 px-3 py-1.5 rounded inline-block">
                                ⏳ 기업 담당자 서명 대기 중
                              </span>
                            </div>
                          )}
                          ...
                          {/* 컨트롤 버튼 (인쇄 제외) */}
                          <div className="flex items-center justify-center gap-2 pt-2.5 no-print">
                            <button
                              type="button"
                              onClick={() => setIsWeekendSignatureCanvasOpen(true)}
                              className="px-3 py-1.5 bg-teal-700 hover:bg-teal-600 text-white rounded font-bold text-xs flex items-center gap-1 shadow-2xs cursor-pointer transition-colors"
                            >
                              <PenTool className="w-3.5 h-3.5" />
                              <span>✍️ 담당자 캔버스 서명</span>
                            </button>
                            {!weekendData.clientVerified && !weekendData.signatureDataUrl && (
                              <button
                                type="button"
                                onClick={() => {
                                  ...
                                  alert(`[기업 확인 승인 완료]\n${company.contactPerson || company.ceoName || '담당자'}님의 확인 서명이 승인되었습니다.`);
                                }}
                                className="px-3 py-1.5 bg-emerald-600 hover:bg-emerald-500 text-white rounded font-bold text-xs flex items-center gap-1 shadow-2xs cursor-pointer transition-colors"
                              >
                            ...
                            {(weekendData.clientVerified || weekendData.signatureDataUrl) && (
                              <button
                                ...
                                className="px-2 py-1 bg-slate-100 hover:bg-slate-200 text-slate-600 rounded text-[11px] font-medium border border-slate-300 cursor-pointer"
                              >
                                서명 초기화
                              </button>
                            )}
                  ...
                  <div className="pt-6 text-center text-[11px] text-slate-400 font-serif">
                    지엠에스씨에스(주) 공인인증원 | GMS CERTIFICATION SERVICE
                  </div>
                </div>
              )}
              ...
              {/* ================================================================= */}
              {/* 인증변경신청서 (F19-002) - 현장 심사 중 작성 및 실시간 연동 */}
              {/* ================================================================= */}
              {activeDocTab === 'cert_change' && (
                <div className="w-full bg-white border border-slate-300 shadow-md p-8 md:p-12 text-slate-900 font-sans a4-page min-h-[1100px] space-y-6 relative flex flex-col justify-between">
                  <div className="space-y-6">
                    {/* 상단 공식 헤더 */}
                    <div className="flex justify-between items-center border-b-2 border-slate-800 pb-2 mb-3">
                    ...
                    <div className="flex justify-between items-center no-print bg-purple-50 border border-purple-200 p-3 rounded-xl">
                      <div className="flex items-center gap-2">
                        <RefreshCcw className="w-4 h-4 text-purple-700 animate-spin-slow" />
                        <span className="text-xs font-bold text-purple-950">현장 인증변경신청서 작성 모드 (심사 중 상호/주소/인증범위 변경 발생 시 작성)</span>
                      </div>
                      <div className="flex items-center gap-2">
                        <button
                          type="button"
                          onClick={handleSaveCertChange}
                          className="px-3.5 py-1.5 bg-purple-700 hover:bg-purple-800 text-white rounded-lg text-xs font-bold transition flex items-center gap-1.5 shadow-sm cursor-pointer"
                        >
                          <Save className="w-3.5 h-3.5" />
                          <span>신청서 저장</span>
                        </button>
                        <button
                          type="button"
                          onClick={() => window.print()}
                          className="px-3 py-1.5 bg-white border border-slate-300 hover:bg-slate-100 text-slate-700 rounded-lg text-xs font-bold transition flex items-center gap-1 cursor-pointer"
                        >
                          <Printer className="w-3.5 h-3.5" />
                          <span>인쇄</span>
                        </button>
                    ...
                    <div className="text-center py-2">
                      ...
                      <p className="text-xs text-slate-500 mt-1 font-serif">
                        (상호, 대표자, 사업장 소재지, 생산품목 및 인증범위 변경 신청서)
                      </p>
                    </div>
                        ...
                        <tbody>
                          ...
                          <tr>
                            ...
                            <td className="p-2 border-r border-slate-300">
                              <input
                                type="text"
                                value={certChangeData.contactPerson}
                                onChange={(e) => setCertChangeData((prev: any) => ({ ...prev, contactPerson: e.target.value }))}
                                className="w-full border border-slate-300 rounded px-1.5 py-0.5 text-xs bg-white"
                                placeholder="담당자 성명"
                              />
                            </td>
                            <th className="bg-slate-100 p-2 border-r border-slate-300 text-left font-semibold">인증번호</th>
                            <td className="p-2 font-mono font-bold text-indigo-900">{certChangeData.certNumber}</td>
                          </tr>
                        </tbody>
                      </table>
                    </div>
                    ...
                    <div className="space-y-2">
                      <h3 className="font-bold text-slate-900 text-xs flex items-center gap-1.5">
                        <span className="w-1.5 h-3 bg-purple-700 inline-block rounded-xs"></span>
                        <span>2. 변경 신청 항목 및 변경 전·후 대비표</span>
                      </h3>
                      <div className="p-2.5 bg-slate-50 border border-slate-300 rounded flex flex-wrap items-center gap-4 text-xs font-semibold">
                        <span className="text-slate-700">변경 구분:</span>
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={certChangeData.changeTypes?.companyName}
                            onChange={(e) => setCertChangeData((prev: any) => ({
                              ...prev,
                              changeTypes: { ...prev.changeTypes, companyName: e.target.checked }
                            }))}
                        ...
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={certChangeData.changeTypes?.address}
                            onChange={(e) => setCertChangeData((prev: any) => ({
                              ...prev,
                              changeTypes: { ...prev.changeTypes, address: e.target.checked }
                            }))}
                            className="rounded text-purple-600 focus:ring-purple-500"
                          />
                          <span>소재지(주소) 변경</span>
                        ...
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={certChangeData.changeTypes?.ceo}
                            onChange={(e) => setCertChangeData((prev: any) => ({
                              ...prev,
                              changeTypes: { ...prev.changeTypes, ceo: e.target.checked }
                            }))}
                            className="rounded text-purple-600 focus:ring-purple-500"
                          />
                          <span>대표자 변경</span>
                        ...
                        <label className="flex items-center gap-1.5 cursor-pointer">
                          <input
                            type="checkbox"
                            checked={certChangeData.changeTypes?.scopeAdd}
                            onChange={(e) => setCertChangeData((prev: any) => ({
                              ...prev,
                              changeTypes: { ...prev.changeTypes, scopeAdd: e.target.checked }
                            }))}
                            className="rounded text-purple-600 focus:ring-purple-500"
                          />
                          <span>생산품목 / 인증범위 변경</span>
                        </label>
                      </div>
                      ...
                      <table className="w-full border-collapse border border-slate-400 text-xs">
                        <thead className="bg-slate-100 border-b border-slate-400 font-bold text-center">
                          <tr>
                            ...
                            <th className="p-2 border-r border-slate-300 w-1/2">변  경  전 (현재 인증등록사항)</th>
                            <th className="p-2 w-1/2">변  경  후 (신청 및 변경 사항)</th>
                          </tr>
                        </thead>
                        <tbody className="divide-y divide-slate-300">
                          <tr>
                            <th className="bg-slate-50 p-2 border-r border-slate-300 text-center font-semibold">상호 (국문/영문)</th>
                            <td className="p-2 border-r border-slate-300 text-slate-600">{activeComp.companyName}</td>
                            <td className="p-2">
                              <input
                                type="text"
                                value={certChangeData.newCompanyNameKo}
                                onChange={(e) => setCertChangeData((prev: any) => ({ ...prev, newCompanyNameKo: e.target.value }))}
                                placeholder="변경 후 국문 상호 입력"
                                className="w-full border border-purple-200 rounded px-2 py-1 text-xs font-bold text-slate-900 bg-purple-50/30"
                              />
                            </td>
                          </tr>
                          <tr>
                            <th className="bg-slate-50 p-2 border-r border-slate-300 text-center font-semibold">사업장 주소</th>
                            <td className="p-2 border-r border-slate-300 text-slate-600">{activeComp.address}</td>
                            <td className="p-2">
                              <input
                                type="text"
                                value={certChangeData.newAddressHeadKo}
                                onChange={(e) => setCertChangeData((prev: any) => ({ ...prev, newAddressHeadKo: e.target.value }))}
                                placeholder="변경 후 사업장 주소 입력"
                                className="w-full border border-purple-200 rounded px-2 py-1 text-xs font-bold text-slate-900 bg-purple-50/30"
                              />
                            </td>
                          </tr>
                          <tr>
                            <th className="bg-slate-50 p-2 border-r border-slate-300 text-center font-semibold">인증 범위 / 품목</th>
                            <td className="p-2 border-r border-slate-300 text-slate-600">{activeComp.scope || '-'}</td>
                            <td className="p-2">
                              <textarea
                                rows={2}
                                value={certChangeData.newScope}
                                onChange={(e) => setCertChangeData((prev: any) => ({ ...prev, newScope: e.target.value }))}
                                placeholder="변경 후 인증 범위/생산품목 상세 기술"
                                className="w-full border border-purple-200 rounded px-2 py-1 text-xs font-medium text-slate-900 bg-purple-50/30 leading-relaxed"
                              />
                            </td>
                          </tr>
                        </tbody>
                    ...
                    <div className="pt-4 border-t border-slate-300 text-center space-y-2">
                      ...
                      <p className="font-bold text-xs text-slate-900">
                        신청일자: {certChangeData.appliedDate}
                      </p>
                      <div className="flex items-center justify-center gap-3 pt-2">
                        <span>신청인: <strong>{activeComp.companyName}</strong> 대표이사 <strong>{activeComp.ceoName}</strong></span>
                        <span className="w-10 h-10 rounded-full border border-dashed border-slate-400 flex items-center justify-center text-[10px] text-slate-400 font-serif">
                          (인)
                        </span>
                      </div>
                    </div>
                    ...
                    {/* 4. 인증원 검토 및 확인란 */}
                    <div className="border border-slate-400 p-3.5 bg-slate-50 space-y-2 rounded">
                      <span className="font-bold text-slate-900 text-xs block">[GMSCS 인증원 심사팀 현장 검토 및 확인란]</span>
                      <div className="grid grid-cols-3 gap-2 text-[11px]">
                        <div>확인방법: <strong>현장 실사 및 증빙 확인 완료 (적합)</strong></div>
                        <div>현장심사팀장: <strong>{auditorForm.name || '남경호'} (서명)</strong></div>
                        <div>사무국 최종 승인: <strong>원장직인 날인 예정</strong></div>
                      </div>
                    </div>
                  ...
                  <div className="pt-4 border-t border-slate-300 text-center space-y-1">
                    <p className="text-sm font-black tracking-widest text-slate-900 font-serif">
                      글로벌매니지먼트시스템인증원(주) 원장 귀하
                    </p>
                    <p className="text-[10px] text-slate-500 font-mono">
                      서울특별시 금천구 디지털로9길 99, 스타밸리 1205호 | Tel: 02-851-2601
                    </p>
                  </div>
                </div>
              )}
      ...
      {/* 기업 담당자 캔버스 전자서명 모달 */}
      {isWeekendSignatureCanvasOpen && (
        <div className="fixed inset-0 z-60 bg-slate-950/70 backdrop-blur-xs flex items-center justify-center p-4">
          <div className="bg-white rounded-2xl shadow-2xl border border-slate-300 max-w-lg w-full overflow-hidden p-6 space-y-4 animate-in fade-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between border-b border-slate-200 pb-3">
              <div className="flex items-center gap-2">
                <PenTool className="w-5 h-5 text-teal-700" />
                <h3 className="text-base font-bold text-slate-900">기업 담당자 전자서명 (휴일근무 확인서)</h3>
              </div>
              <button
                type="button"
                onClick={() => setIsWeekendSignatureCanvasOpen(false)}
            ...
            <p className="text-xs text-slate-600">
              확인자: <strong className="text-slate-900">{company.contactPerson || company.ceoName || '기업 담당자'}</strong> ({(company as any).contactPosition || '담당자'}) 귀하의 전자 서명을 아래 캔버스에 마우스 또는 터치로 직접 작성해주세요.
            </p>
            <SignatureCanvas
              signerTitle={`기업 담당자: ${company.contactPerson || company.ceoName || '담당자'}`}
              onSave={(dataUrl) => {
                const now = new Date().toLocaleString();
                setWeekendData(prev => ({
                  ...prev,
                ...
                setIsWeekendSignatureCanvasOpen(false);
                alert(`[기업 전자서명 완료]\n${company.contactPerson || company.ceoName || '담당자'}님의 캔버스 전자서명이 성공적으로 등록되었습니다.`);
              }}
              onCancel={() => setIsWeekendSignatureCanvasOpen(false)}
            />
          </div>
        </div>
      )}
